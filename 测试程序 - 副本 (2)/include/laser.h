...
inline bool is_stable(int current_value) {
    ...
    for (int i = 1; i < FILTER_SIZE; i++) {
        if (distance_history[i] > 0) {
            if (distance_history[i] > max_val) max_val = distance_history[i];
            if (distance_history[i] < min_val) min_val = distance_history[i];
        }
    }
        ...
        if (g_rx_cnt > 0) {
            for (int i = 1; i < g_rx_cnt; i++) {
                if ((g_rx_buf[i-1] == '\r' && g_rx_buf[i] == '\n') || 
                    ...
                    while (*num_pos && !isdigit(*num_pos)) num_pos++;
                    if (*num_pos) {

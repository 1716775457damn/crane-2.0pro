...
#include "laser.h"
#include "system_health.h" // 新增系统健康监控头文件
...
void setup()
{
  // 初始化串口
  Serial.begin(115200);
  // 不在这里初始化Serial1，让chassis_serial_init()来处理
  ...
  // 初始化高级电机控制状态
  initAllAdvancedMotors();
  debugPrintAll("Advanced motor control initialized");

  // 初始化底盘控制串口
  chassis_serial_init();
  chassis_init_state();
  debugPrintAll("Chassis control initialized with enhanced safety features");
...
void loop()
{
  ...
  // Update advanced stepper motors with PID control
  updateAdvancedSteppers();

  // Update trapezoidal stepper motors (acceleration-constant-deceleration)
  updateTrapezoidalSteppers();

  // Update chassis smooth speed control and safety checks
  chassis_update_smooth_speed();
...
void handleCommand(String cmd)
{
  ...
  if (cmd == "chassis_status")
  {
    chassis_print_detailed_status();
    return;
  }
  ...
  if (cmd == "reset_errors")
  {
    ...
    debugPrintAll("Error count reset");
    return;
  }

  if (cmd == "performance_report")
  {
    printPerformanceReport();
    return;
  }

  if (cmd == "reset_performance")
  {
    resetPerformanceMetrics();
    return;
  }

  if (cmd == "memory_report")
  {
    printMemoryReport();
    return;
  }
  ...
  if (cmd.startsWith("chassis_move,"))
  {
    ...
    if (firstComma > 0 && secondComma > firstComma)
    {
      ...
      debugPrintAll("Enhanced chassis " + direction + " at target speed " + String(speed));
    }
    else
    {
  ...
  // 底盘安全配置命令
  if (cmd.startsWith("chassis_safety,"))
  {
    int firstComma = cmd.indexOf(',');
    int secondComma = cmd.indexOf(',', firstComma + 1);
    ...
    int sixthComma = cmd.indexOf(',', fifthComma + 1);

    if (firstComma > 0 && secondComma > firstComma && thirdComma > secondComma &&
        fourthComma > thirdComma && fifthComma > fourthComma && sixthComma > fifthComma)
    {
      float acceleration = cmd.substring(firstComma + 1, secondComma).toFloat();
      float deceleration = cmd.substring(secondComma + 1, thirdComma).toFloat();
      uint8_t minSpeed = cmd.substring(thirdComma + 1, fourthComma).toInt();
      uint8_t maxSpeed = cmd.substring(fourthComma + 1, fifthComma).toInt();
      unsigned long maxTime = cmd.substring(fifthComma + 1, sixthComma).toInt() * 1000; // 转换为毫秒
      unsigned long timeout = cmd.substring(sixthComma + 1).toInt() * 1000;             // 转换为毫秒

      chassis_configure_safety(acceleration, deceleration, minSpeed, maxSpeed, maxTime, timeout);
    }
    else
    {
      debugPrintAll("Format error, should be: chassis_safety,ACCEL,DECEL,MIN_SPEED,MAX_SPEED,MAX_TIME_S,TIMEOUT_S");
      debugPrintAll("Example: chassis_safety,50,80,5,100,30,2");
    }
    return;
  }

  if (cmd.startsWith("chassis_safety_enable,"))
  {
    int commaIndex = cmd.indexOf(',');
    if (commaIndex > 0)
    {
      bool enabled = cmd.substring(commaIndex + 1).toInt() == 1;
      chassis_set_safety_enabled(enabled);
    }
    else
    {
      debugPrintAll("Format error, should be: chassis_safety_enable,1 or chassis_safety_enable,0");
    }
    return;
  }
  ...
  if (cmd == "test_chassis")
  {
    debugPrintAll("=== Chassis Communication Test (TX-Only) ===");
    debugPrintAll("Testing chassis serial communication...");
    debugPrintAll("Note: Chassis is TX-only, no response expected");

    // 测试串口状态
    debugPrintAll("Serial1 TX buffer available: " + String(Serial1.availableForWrite()));
    debugPrintAll("Baud rate: 9600, TX pin: 19");

    // 测试前后移动（使用正确的命令格式）
    debugPrintAll("Test 1: Forward command '5,800,30'");
    Serial1.println("5,800,30");
    Serial1.flush();
    debugPrintAll("Forward command sent, waiting 3 seconds...");
    delay(3000);

    debugPrintAll("Test 2: Stop command '0,0,30'");
    Serial1.println("0,0,30");
    Serial1.flush();
    debugPrintAll("Stop command sent, waiting 1 second...");
    delay(1000);

    debugPrintAll("Test 3: Backward command '6,800,30'");
    Serial1.println("6,800,30");
    Serial1.flush();
    debugPrintAll("Backward command sent, waiting 3 seconds...");
    delay(3000);

    debugPrintAll("Test 4: Final stop command '0,0,30'");
    Serial1.println("0,0,30");
    Serial1.flush();
    debugPrintAll("Final stop sent, waiting 1 second...");
    delay(1000);

    debugPrintAll("Chassis test completed (TX-only communication)");
    debugPrintAll("If chassis moved, communication is working correctly");
    debugPrintAll("===============================");
    return;
  }

  // 高级步进电机控制命令
  if (cmd.startsWith("stepper_advanced,"))
  {
    int firstComma = cmd.indexOf(',');
    int secondComma = cmd.indexOf(',', firstComma + 1);
    int thirdComma = cmd.indexOf(',', secondComma + 1);
    ...
    if (firstComma > 0 && secondComma > firstComma)
    {
      ...
      if (stepper_move_to_position_advanced(motorNumber, target_position, speed))
      {
        debugPrintAll("Advanced motor " + String(motorNumber) + " moving to position " + String(target_position) + " with PID control at speed " + String(speed));
      }
      else
      {
        debugPrintAll("Failed to start advanced motor " + String(motorNumber));
      }
    }
    else
    {
      debugPrintAll("Format error, should be: stepper_advanced,MOTOR,POSITION[,SPEED]");
      debugPrintAll("Example: stepper_advanced,1,200,150 (motor 1 to position 200 with PID at speed 150)");
    }
    return;
  }

  // 设置PID参数命令
  if (cmd.startsWith("stepper_pid,"))
  {
    int firstComma = cmd.indexOf(',');
    int secondComma = cmd.indexOf(',', firstComma + 1);
    int thirdComma = cmd.indexOf(',', secondComma + 1);
    ...
    if (firstComma > 0 && secondComma > firstComma && thirdComma > secondComma && fourthComma > thirdComma)
    {
      ...
      stepper_set_pid_params(motorNumber, kp, ki, kd);
      debugPrintAll("PID parameters set for motor " + String(motorNumber));
    }
    else
    {
      debugPrintAll("Format error, should be: stepper_pid,MOTOR,KP,KI,KD");
      debugPrintAll("Example: stepper_pid,1,0.8,0.1,0.2");
    }
    return;
  }

  // 设置加速度参数命令
  if (cmd.startsWith("stepper_accel,"))
  {
    int firstComma = cmd.indexOf(',');
    int secondComma = cmd.indexOf(',', firstComma + 1);
    int thirdComma = cmd.indexOf(',', secondComma + 1);
    ...
    {
      int motorNumber = cmd.substring(firstComma + 1, secondComma).toInt();
      float acceleration = cmd.substring(secondComma + 1, thirdComma).toFloat();
      float deceleration = cmd.substring(thirdComma + 1).toFloat();

      stepper_set_acceleration(motorNumber, acceleration, deceleration);
      debugPrintAll("Acceleration parameters set for motor " + String(motorNumber));
    }
    else
    {
      debugPrintAll("Format error, should be: stepper_accel,MOTOR,ACCEL,DECEL");
      debugPrintAll("Example: stepper_accel,1,3.0,2.5");
    }
    return;
  }

  // 底盘直接控制命令：chassis_cmd,COMMAND,STEPS,SPEED
  if (cmd.startsWith("chassis_cmd,"))
  {
    int firstComma = cmd.indexOf(',');
    int secondComma = cmd.indexOf(',', firstComma + 1);
    int thirdComma = cmd.indexOf(',', secondComma + 1);
    ...
    if (firstComma > 0 && secondComma > firstComma && thirdComma > secondComma)
    {
      uint8_t command = cmd.substring(firstComma + 1, secondComma).toInt();
      uint16_t steps = cmd.substring(secondComma + 1, thirdComma).toInt();
      uint8_t speed = cmd.substring(thirdComma + 1).toInt();

      // 验证命令类型
      if (command != 0 && command != 5 && command != 6)
      {
        debugPrintAll("Invalid command! Use: 5=forward, 6=backward, 0=stop");
        return;
      }
      ...
      // 静默发送到19号串口
      String chassis_cmd = String(command) + "," + String(steps) + "," + String(speed);
      Serial1.println(chassis_cmd);
      Serial1.flush();

      String cmd_name = (command == 5) ? "FORWARD" : (command == 6) ? "BACKWARD"
                                                                    : "STOP";
      debugPrintAll("Chassis TX: " + chassis_cmd + " (" + cmd_name + ")");
    }
    else
    {
      debugPrintAll("Format: chassis_cmd,COMMAND,STEPS,SPEED");
      debugPrintAll("Commands: 5=forward, 6=backward, 0=stop");
      debugPrintAll("Example: chassis_cmd,5,1000,30 (forward 1000 steps at speed 30)");
    }
    return;
  }

  // 梯形速度控制命令
  if (cmd.startsWith("stepper_trap,"))
  {
    int firstComma = cmd.indexOf(',');
    int secondComma = cmd.indexOf(',', firstComma + 1);
    int thirdComma = cmd.indexOf(',', secondComma + 1);
    ...
    if (firstComma > 0 && secondComma > firstComma)
    {
      ...
      if (stepper_move_to_position_trapezoidal(motorNumber, target_position, speed))
      {
        debugPrintAll("Trapezoidal motor " + String(motorNumber) + " moving to position " + String(target_position) + " with acceleration control at speed " + String(speed));
      }
      else
      {
        debugPrintAll("Failed to start trapezoidal motor " + String(motorNumber));
      }
    }
    else
    {
      debugPrintAll("Format error, should be: stepper_trap,MOTOR,POSITION[,SPEED]");
      debugPrintAll("Example: stepper_trap,1,1000,150 (motor 1 to position 1000 with trapezoidal speed at 150)");
    }
    return;
  }

  // 设置梯形控制参数命令
  if (cmd.startsWith("stepper_trap_params,"))
  {
    int firstComma = cmd.indexOf(',');
    int secondComma = cmd.indexOf(',', firstComma + 1);
    int thirdComma = cmd.indexOf(',', secondComma + 1);
    ...
    {
      int motorNumber = cmd.substring(firstComma + 1, secondComma).toInt();
      float maxSpeed = cmd.substring(secondComma + 1, thirdComma).toFloat();
      float acceleration = cmd.substring(thirdComma + 1, fourthComma).toFloat();
      float deceleration = cmd.substring(fourthComma + 1).toFloat();

      stepper_set_trapezoidal_params(motorNumber, maxSpeed, acceleration, deceleration);
      debugPrintAll("Trapezoidal parameters set for motor " + String(motorNumber));
    }
    else
    {
      debugPrintAll("Format error, should be: stepper_trap_params,MOTOR,MAX_SPEED,ACCEL,DECEL");
      debugPrintAll("Example: stepper_trap_params,1,1500,800,600");
    }
    return;
  }
...
void showHelp()
{
  ...
  debugPrintAll("health_report         Show comprehensive system health report");
  debugPrintAll("performance_report    Show performance metrics and timing");
  debugPrintAll("memory_report         Show detailed memory usage statistics");
  debugPrintAll("restart               Restart the system");
  debugPrintAll("reset_errors          Reset error count and clear last error");
  debugPrintAll("reset_performance     Reset performance metrics counters");
  ...
  debugPrintAll("");
  debugPrintAll("=== Enhanced Chassis Control ===");
  ...
  debugPrintAll("chassis_move,DIR,S    Enhanced chassis control with smooth acceleration");
  debugPrintAll("                      (DIR: f/b/l/r/s, S: 1-100)");
  debugPrintAll("chassis_cmd,C,ST,SP   Raw command control (C: 5/6/0, ST: steps, SP: speed)");
  debugPrintAll("chassis_status        Show detailed chassis status and safety info");
  debugPrintAll("emergency_stop        Activate emergency stop (immediate halt)");
  debugPrintAll("clear_emergency       Clear emergency stop condition");
  debugPrintAll("chassis_safety,A,D,MIN,MAX,TIME,TIMEOUT  Configure safety parameters");
  debugPrintAll("                      (A: accel, D: decel, MIN/MAX: speed %, TIME/TIMEOUT: seconds)");
  debugPrintAll("chassis_safety_enable,1/0  Enable/disable safety features");
  debugPrintAll("");
  ...
  debugPrintAll("");
  debugPrintAll("=== Advanced Stepper Control (PID + S-Curve) ===");
  debugPrintAll("stepper_advanced,M,P[,S]  Advanced move with PID control");
  debugPrintAll("stepper_pid,M,KP,KI,KD    Set PID parameters for motor M");
  debugPrintAll("stepper_accel,M,A,D       Set acceleration/deceleration for motor M");
  debugPrintAll("");
  debugPrintAll("=== Trapezoidal Speed Control (Accel-Constant-Decel) ===");
  debugPrintAll("stepper_trap,M,P[,S]      Trapezoidal move (smooth acceleration)");
  debugPrintAll("stepper_trap_params,M,MS,A,D  Set trapezoidal parameters");
  debugPrintAll("");
...
SystemConfig systemConfig = {
    ...
    .temperatureThreshold = DEFAULT_TEMP_THRESHOLD,
    .safetyInterval = DEFAULT_SAFETY_INTERVAL};
...
void performSafetyChecks()
{
  ...
  bool previousHealthStatus = checkSystemHealth();

  // 1. 内存检查
  uint32_t freeHeap = ESP.getFreeHeap();
  systemHealth.memoryOK = (freeHeap > 15000); // 15KB 最小内存
  if (!systemHealth.memoryOK && previousHealthStatus)
  {
    handleCriticalError("Critical memory shortage: " + String(freeHeap) + " bytes");
  }

  // 2. 温度检查
  float temperature = temperatureRead();
  systemHealth.temperatureOK = (temperature < 80.0); // 80°C 最高温度
  if (!systemHealth.temperatureOK && previousHealthStatus)
  {
    handleCriticalError("CPU overheating: " + String(temperature) + "°C");
  }

  // 3. 通信检查
  systemHealth.communicationOK = (WiFi.softAPgetStationNum() >= 0); // WiFi AP 正常

  // 4. 电机安全检查
  systemHealth.motorsOK = true;
  for (int i = 1; i <= 4; i++)
  {
    // 检查电机是否卡死或过载（这里简化为检查运行时间）
    ...
    {
    case 1:
      motorRunning = motor1State.isRunning;
      runTime = motorRunning ? (millis() - motor1State.startTime) : 0;
      break;
    case 2:
      motorRunning = motor2State.isRunning;
      runTime = motorRunning ? (millis() - motor2State.startTime) : 0;
      break;
    case 3:
      motorRunning = motor3State.isRunning;
      runTime = motorRunning ? (millis() - motor3State.startTime) : 0;
      break;
    case 4:
      motorRunning = motor4State.isRunning;
      runTime = motorRunning ? (millis() - motor4State.startTime) : 0;
      break;
    }

    // 如果电机运行超过5分钟，可能有问题
    if (motorRunning && runTime > 300000)
    {
      systemHealth.motorsOK = false;
      handleSystemError("Motor " + String(i) + " running too long: " + String(runTime / 1000) + "s");
    }
  }

  // 5. 传感器检查
  systemHealth.sensorsOK = true;
  // 检查激光传感器
  if (laser_initialized)
  {
    int laserReading = getLaserDistanceOptimized();
    if (laserReading < 0)
    {
      systemHealth.sensorsOK = false;
      // 不是关键错误，只记录
      logSystemEvent("Laser sensor reading failed");
    }
  }

  // 6. 底盘安全检查（已在chassis.h中实现）
  ...
  {
    logSystemEvent("System health check failed");
  }
}
...
bool checkSystemHealth()
{
  return systemHealth.memoryOK &&
         systemHealth.temperatureOK &&
         systemHealth.communicationOK &&
         systemHealth.motorsOK &&
         systemHealth.sensorsOK;
}
...
void handleCriticalError(const String &errorMsg)
{
  ...
  systemHealth.lastError = errorMsg;

  debugPrintAll("CRITICAL ERROR: " + errorMsg);
  debugPrintAll("Error count: " + String(systemHealth.errorCount));
  ...
  for (int i = 1; i <= 4; i++)
  {
    ...
    digitalWrite(sleepPin, LOW); // 禁用电机
  }

  // 如果错误太多，重启系统
  if (systemHealth.errorCount > 10)
  {
    debugPrintAll("Too many critical errors, restarting system...");
    delay(2000);
    ESP.restart();
  }

  logSystemEvent("Critical error handled: " + errorMsg);
}
...
void logSystemEvent(const String &event)
{
  ...
  debugPrintAll("[" + timestamp + "s] EVENT: " + event);

  // 这里可以添加更复杂的日志记录，比如写入EEPROM或SD卡
}
...
{
  debugPrintAll("=== System Health Report ===");
  debugPrintAll("Memory: " + String(systemHealth.memoryOK ? "OK" : "CRITICAL"));
  debugPrintAll("Temperature: " + String(systemHealth.temperatureOK ? "OK" : "CRITICAL"));
  debugPrintAll("Communication: " + String(systemHealth.communicationOK ? "OK" : "WARNING"));
  debugPrintAll("Motors: " + String(systemHealth.motorsOK ? "OK" : "WARNING"));
  debugPrintAll("Sensors: " + String(systemHealth.sensorsOK ? "OK" : "WARNING"));
  debugPrintAll("Overall Health: " + String(checkSystemHealth() ? "HEALTHY" : "UNHEALTHY"));
  debugPrintAll("Error Count: " + String(systemHealth.errorCount));
  debugPrintAll("Last Error: " + systemHealth.lastError);
  debugPrintAll("Last Check: " + String((millis() - systemHealth.lastHealthCheck) / 1000) + "s ago");
  debugPrintAll("============================");
}
...
void updatePerformanceMetrics(unsigned long loopStartTime)
{
  ...
  performanceMetrics.loopTime = currentLoopTime;
  performanceMetrics.loopCount++;

  // 更新最大循环时间
  if (currentLoopTime > performanceMetrics.maxLoopTime)
  {
    performanceMetrics.maxLoopTime = currentLoopTime;
  }

  // 计算平均循环时间（使用移动平均）
  if (performanceMetrics.loopCount == 1)
  {
    performanceMetrics.avgLoopTime = currentLoopTime;
  }
  else
  {
    // 简单移动平均，权重为0.1
    performanceMetrics.avgLoopTime = (performanceMetrics.avgLoopTime * 9 + currentLoopTime) / 10;
  }

  // 检查性能警告
  if (currentLoopTime > MAX_LOOP_TIME_WARNING)
  {
    logSystemEvent("Performance warning: Loop time " + String(currentLoopTime) + "μs exceeds threshold");
  }
}
...
{
  debugPrintAll("=== Performance Report ===");
  debugPrintAll("Loop Count: " + String(performanceMetrics.loopCount));
  debugPrintAll("Current Loop Time: " + String(performanceMetrics.loopTime) + "μs");
  debugPrintAll("Average Loop Time: " + String(performanceMetrics.avgLoopTime) + "μs");
  debugPrintAll("Maximum Loop Time: " + String(performanceMetrics.maxLoopTime) + "μs");
  debugPrintAll("Web Response Time: " + String(performanceMetrics.webResponseTime) + "ms");
  debugPrintAll("Chassis Response Time: " + String(performanceMetrics.chassisResponseTime) + "ms");
  debugPrintAll("Safety Check Time: " + String(performanceMetrics.safetyCheckTime) + "μs");
  debugPrintAll("==========================");
}
...
{
  performanceMetrics.loopTime = 0;
  performanceMetrics.webResponseTime = 0;
  performanceMetrics.chassisResponseTime = 0;
  performanceMetrics.safetyCheckTime = 0;
  performanceMetrics.maxLoopTime = 0;
  performanceMetrics.avgLoopTime = 0;
  performanceMetrics.loopCount = 0;
  debugPrintAll("Performance metrics reset");
}
...
{
  MemoryUsage usage;
  usage.totalHeap = ESP.getHeapSize();
  usage.freeHeap = ESP.getFreeHeap();
  usage.minFreeHeap = ESP.getMinFreeHeap();
  usage.maxAllocHeap = ESP.getMaxAllocHeap();

// PSRAM信息（如果可用）
#ifdef BOARD_HAS_PSRAM
  usage.psramTotal = ESP.getPsramSize();
  usage.psramFree = ESP.getFreePsram();
#else
  usage.psramTotal = 0;
  usage.psramFree = 0;
#endif
...
{
  uint32_t freeHeap = ESP.getFreeHeap();
  return freeHeap > systemConfig.memoryThreshold;
}
...
void printMemoryReport()
{
  ...
  debugPrintAll("=== Memory Report ===");
  debugPrintAll("Total Heap: " + formatMemorySize(usage.totalHeap));
  debugPrintAll("Free Heap: " + formatMemorySize(usage.freeHeap));
  debugPrintAll("Min Free Heap: " + formatMemorySize(usage.minFreeHeap));
  debugPrintAll("Max Alloc Heap: " + formatMemorySize(usage.maxAllocHeap));

  if (usage.psramTotal > 0)
  {
    debugPrintAll("Total PSRAM: " + formatMemorySize(usage.psramTotal));
    debugPrintAll("Free PSRAM: " + formatMemorySize(usage.psramFree));
  }

  debugPrintAll("Memory Status: " + String(isMemoryUsageSafe() ? "SAFE" : "WARNING"));
  debugPrintAll("====================");
}